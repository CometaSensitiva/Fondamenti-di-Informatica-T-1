...
int findPosMax(elementArray a, int dim){
    ...
    for(i=0; i<dim; i++){
        //Se l'elemento i-esimo è maggiore secondo la funzione compare, i diventa la posMax
            if(compareElement(a[posMax], a[i])<0)
                posMax=i;
    }
    //Ritorna la posMax
    return posMax;
...
void insertionSort(elementArray a, int dim){
    ...
    for (i=1; i<dim;i++){
        ...
        assign(&key,a[i]);
        j= i-1;
        while(j>= 0 && compareElement(a[j],key)>0){
            assign(&a[j+1],a[j]);
            j=j-1;
        }
...
void mergeSortedArrays(elementArray a, int l, int m, int r) {
    ...
    tempLeft = (element*)malloc(leftLength*sizeof(element));
    tempRight = (element*)malloc(rightLength*sizeof(element));
    ...
    for (i = 0, j = 0, k = l; k <= r; k++)
    {
        ...
        if ((i < leftLength) && (j >= rightLength || compareElement(tempLeft[i], tempRight[j]) <= 0)) {
...
int partition(elementArray a, int low, int high){
    ...
    for(int j=low;j<high;j++){
        if(compareElement(a[j],pivotValue)<=0){
            swap(&a[i], &a[j]);
            i++;
        }
    }
